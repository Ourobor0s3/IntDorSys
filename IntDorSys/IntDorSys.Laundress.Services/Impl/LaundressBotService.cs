    ...
    internal sealed class LaundressBotService : ILaundressBotService, ILaundressBotMenuService, ILaundressBotBookingService, ILaundressBotNotificationService
    {
        ...
        public async Task SendDatesForDeleteAsync(long chatId, int messageId = 0, CancellationToken ct = default)
        {
            try
            {
                ...
                var inlineKeyboard = dates.Select(x => new[]
                        { KeyboardButtons.InlineButton("DelDate", x.ToString("yyyy-MM-dd"), x.ToString("dd.MM.yyyy")) })
                    .ToList();
        ...
        public async Task SendFreeDateAsync(long chatId, int messageId = 0, CancellationToken ct = default)
        {
            try
            {
                ...
                var inlineKeyboard = dates.Select(x => new[]
                        { KeyboardButtons.InlineButton("UseDate", x.ToString("yyyy-MM-dd"), x.ToString("dd.MM.yyyy")) })
                    .ToList();
